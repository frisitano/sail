...
def check(plan: dict[str, Any]) -> dict[str, int]:
    ...
    require(plan.get("schema_version") == "1.1.0", "unsupported schema version")
    ...
    require(
        plan.get("configuration", {}).get("narrowing_policy") in {"checked", "proven", "all"},
        "missing or invalid narrowing policy",
    )
    ...
    for clone in clones:
        ...
        require(
            all(conversion.get("validation") in {"checked", "proven", "assumed"} for conversion in conversions),
            f"{clone.get('id')}: invalid conversion validation",
        )
    ...
    require(isinstance(assumptions, list), "assumptions must be an array")
    require_sorted_unique(assumptions, "assumptions")
    assumption_kinds = {assumption.get("kind") for assumption in assumptions}
    has_assumed_conversion = any(
        conversion.get("validation") == "assumed"
        for clone in clones
        for conversion in clone["conversions"]
    )
    require(
        ("unchecked_narrowing" in assumption_kinds) == has_assumed_conversion,
        "unchecked narrowing assumption does not match assumed conversions",
    )
    has_unresolved_extern = any(
        call.get("extern") is True for clone in clones for call in clone["call_edges"]
    ) and bool(unresolved)
    require(
        ("extern_contract" in assumption_kinds) == has_unresolved_extern,
        "extern assumption does not match unresolved extern contracts",
    )
