...
sed -n '/^uint8_t one_use_guard_in_composed_condition(/,/^}/p' \
  "$SPEC_SOURCE/machine.c" > "$TMP_DIR/one_use_guard_in_composed_condition.c"
grep -Eq 'if \(.*value != UINT8_C\(0\).*\|\| fallback\)' \
  "$TMP_DIR/one_use_guard_in_composed_condition.c"
