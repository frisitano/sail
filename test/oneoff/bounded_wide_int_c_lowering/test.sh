...
# u256 operands can sum to 257 bits, so this operation must remain arbitrary
# precision rather than silently becoming modular u256 arithmetic.
sed -n '/^void zadd_widen_256(/,/^}/p' "$OUT.c" > "$TMP_DIR/add_widen_256.c"
grep -Fq 'add_int(' "$TMP_DIR/add_widen_256.c"
