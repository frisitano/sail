...
* `--c-no-mangle` preserves valid Sail source identifiers where possible.
    Compiler-generated locals, structural types, monomorphized names, and
    invalid or reserved identifiers continue to use z-encoding and the normal
    collision suffixes.

* `--c-specialize` preserve proof-backed fixed integer representations across
    C function boundaries and specialize local function bodies as required.

* `--c-specialize-log` report representation demands, semantic argument
    bounds, iterative worklist depth, and elapsed compiler time while
    `--c-specialize` runs.

* `--c-require-bounded-int` reject generated definitions that still require
    arbitrary-precision `sail_int`. The diagnostic identifies the owning Sail
    definition so its parameter, result, local value, or container element can
    be given a finite semantic bound. Use this only for targets that require an
    entirely fixed-width integer model.

* `--c-optimized-include-dir` select the include root containing handwritten
    headers used by an optimized-model extraction.

* `--c-optimized-source-root` split an optimized-model extraction by Sail
    source file, preserving each file's path relative to the supplied source
    root beneath the generated `include/PACKAGE/spec/` and `src/spec/`
    directories. The compiler also writes the generated C paths, in build
    order and relative to `src/spec/`, to `src/spec/sources.list`. Without
    this option, optimized-model output remains split by semantic Sail module.
    This option requires `--c-optimized-model`.

* `--c-optimized-external-type TYPE=HEADER` declare that `TYPE`'s final C
    declaration is owned by `HEADER`, relative to the optimized include root.
    This repeatable option requires `--c-optimized-model`. The generated module
    headers include `HEADER` and omit Sail's declaration of `TYPE`; generated
    functions continue to use the external type directly. Sail trusts the
    external declaration as an ABI contract and does not parse it or emit
    layout assertions. For example:

[source,shell]
----
sail -c --c-optimized-model \
  --c-optimized-include-dir ffi/optimized/include \
  --c-optimized-external-type pair=evmsail/host/types.h \
  model.sail_project
----

    Compile the generated sources with the same root on the C include path,
    for example `-I ffi/optimized/include`.

* `--c-optimized-byte-pointer-field TYPE.FIELD=ADAPTER` represents an integer
    field of a generated or externally owned record as `uint8_t *` in
    optimized C. The
    source Sail field remains an integer, so other backends and proof
    extraction retain its semantic representation. Generated construction
    converts an integer offset with `ADAPTER((uint64_t) offset)`; field
    equality, ordering, addition, and subtraction operate directly on the C
    pointer representation. This repeatable option requires
    `--c-optimized-model`. When `TYPE` is also supplied by
    `--c-optimized-external-type TYPE=HEADER`, that header must declare
    `FIELD` as `uint8_t *`. In either case the optimized include surface must
    declare the adapter function.

* `--c-optimized-byte-pointer-type TYPE=ADAPTER` gives every occurrence of a
    nominal integer `TYPE` the `uint8_t *` representation in optimized C. Use
    this for abstract handles whose optimized backend representation is a
    borrowed byte pointer, including direct extern arguments and results. The
    Sail type and all non-C backends retain the integer semantics. Integer
    values crossing into the pointer representation are converted with
    `ADAPTER((uint64_t) value)`, while zero is emitted as `NULL`. This
    repeatable option requires `--c-optimized-model`.

The structured `$target_name` directive can override a source identifier's
generated spelling for one backend. The C backend currently supports it with
`backend = "c"`; directives for other backends are ignored. For example:
...
$target_name {
  backend = "c",
  id = CREATE,
  target = "opcode_CREATE"
}
----

This is useful with `--c-no-mangle` when an otherwise readable Sail identifier
collides with a C runtime or FFI identifier. The existing C-specific
`$c_override` directive remains supported.
